...
def get_by_id(id: int) -> db.User:
    return user_repo.get_by_id(id)


def get_by_email(email: str) -> db.User:
    return user_repo.get_by_email(email.lower().strip())


def create(
    name: str, surname: str, role: db.User.Role, email: str, password: str
) -> db.User:
...
def update(
    id: int, name: str, surname: str, role: db.User.Role, email: str, password: str
) -> None:
    name = formating.format_string(name)
    surname = formating.format_string(surname)
    email = formating.format_string(email)
...
def update_name_surname(id: int, name: str, surname: str) -> None:
    ...
    name = formating.format_string(name)
    surname = formating.format_string(surname)
    user_repo.update(user.id, name, surname, user.role, user.email, user.password)
...
def update_password(id: int, new_password: str) -> None:
    ...
    user_repo.update(
        user.id, user.name, user.surname, user.role, user.email, new_pass_hash
    )
...
def reset_password(id: int) -> str:
    ...
    user_repo.update(user.id, user.name, user.surname, user.role, user.email, pass_hash)
